...
- **Animated ECG heartbeat** — the glowing pulse line under the clock is what makes it special, which matches the futuristic theme of the website.
- **Live clock & date** —The  big glowing Orbitron clock with an hour-based greeting looks intercative and futuristic.
...
Step 2. `npm install`
Step 3. Get a free key at [api.nasa.gov](https://api.nasa.gov) — it's emailed to you instantly
...
The repository includes `.github/workflows/deploy.yml` which automatically deploys on every push to `main`:

Step 1. Create a repo named exactly `apod` (matches `vite.config.js` base path)
Step 2. `git init`, add remote, commit, push
Step 3. Add your key as a repo secret: Settings → Secrets and variables → Actions → **`VITE_NASA_API_KEY`**
